...
interface RoomDao {
    ...
    @Query("SELECT EXISTS (SELECT 1 FROM responses WHERE userId =:userId AND indicatorId =:indicatorId)")
    ...
    @Query("SELECT EXISTS (SELECT 1 FROM comments WHERE userId =:userId AND indicatorId =:indicatorId)")
    ...
    @Query("SELECT EXISTS (SELECT 1 FROM submissions WHERE userId =:userId AND date =:date AND status =:status)")
